...
def test_product_creation_with_fixture(sample_product):
...
def test_multiple_products_with_parametrized_fixture(product_with_params):
...
def test_product_list_operations(product_list):
...
def test_expensive_setup(expensive_product_setup):
...
def test_temporary_product_operations(temporary_product):
...
def test_combined_fixtures(sample_product, product_list):
...
def test_dynamic_product_creation():
    ...
    cheap_product = create_test_product("Дешёвый товар", 10.0)
    expensive_product = create_test_product("Дорогой товар", 10000.0)
